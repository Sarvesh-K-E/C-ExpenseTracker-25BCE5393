...
function wasm_pop_char() { if (typeof window !== 'undefined' && typeof window.__wasmPopChar === 'function') { return window.__wasmPopChar(); } return -1; }
function wasm_has_char() { if (typeof window !== 'undefined' && typeof window.__wasmHasChar === 'function') { return window.__wasmHasChar() ? 1 : 0; } return 0; }
function wasm_set_line_mode(mode) { if (typeof window !== 'undefined' && typeof window.__wasmSetLineMode === 'function') { window.__wasmSetLineMode(mode ? true : false); } }
function wasm_set_waiting_input(waiting) { if (typeof window !== 'undefined' && typeof window.__wasmSetWaitingInput === 'function') { window.__wasmSetWaitingInput(waiting ? true : false); } }
function flushInput() { if (window.flushInput) window.flushInput(); }
function __asyncjs__call_js_import() { return Asyncify.handleAsync(async () => { try { if (!window.triggerCsvImport) { console.error("CSV Import bridge: triggerCsvImport not found"); return 0; } const result = await window.triggerCsvImport(); return result ? 1 : 0; } catch (e) { console.error("CSV Import bridge error:", e); return 0; } }); }
function __asyncjs__call_js_ai(summary,mode) { return Asyncify.handleAsync(async () => { try { if (!window.ext_callAI) { console.error("AI bridge: ext_callAI not found"); return 0; } const result = await window.ext_callAI(summary, mode); return result ? 1 : 0; } catch (e) { console.error("AI bridge error:", e); return 0; } }); }
...
var wasmImports = {
  /** @export */
  __asyncjs__call_js_ai,
  /** @export */
  __asyncjs__call_js_import,
  /** @export */
